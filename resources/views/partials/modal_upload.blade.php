    ...
    fetch('{{ route("meses.disponibles") }}')
        ...
        .then(response => {
            if (response.success && response.data.length > 0) {
                ...
                document.getElementById('selectorPeriodo').innerHTML = html;
                
                const ultimo = response.data[0];
                const valor = ultimo.anio + '-' + String(ultimo.mes).padStart(2, '0');
                document.getElementById('selectorPeriodo').value = valor;
                window.periodoActual = valor;
                if (typeof cargarDatosPorPeriodo === 'function') {
                    cargarDatosPorPeriodo(ultimo.anio, ultimo.mes);
                }
                console.log('✅ Meses cargados:', response.data.length);
            } else {
                document.getElementById('selectorPeriodo').innerHTML = '<option value="">Sin datos</option>';
            }
        })
        .catch(error => {
            console.error('❌ Error al cargar meses:', error);
            document.getElementById('selectorPeriodo').innerHTML = '<option value="">Error al cargar</option>';
        });
}
    ...
    btnSubir.addEventListener('click', function(e) {
        ...
        xhr.onload = function() {
            ...
            if (xhr.status === 419) {
                ...
                progressBarInner.textContent = '0%';
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-cloud-upload-alt me-2"></i>Subir Archivo';
                return;
            }
